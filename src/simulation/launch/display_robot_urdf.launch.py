...
def generate_launch_description():
    ...
    default_model_path = os.path.join(default_package_path, 'urdf', 'base_robot.urdf')
    ...
    command_urdf_content = Command(['cat ', LaunchConfiguration('urdf_path')])
    ...
    executable='rviz2',
    output='screen',
    arguments=['-d', os.path.join(default_package_path, 'config', 'rviz2_config.rviz')]
    )
